					...
					<div class="list-content">
						<div class="list-body">
							<p style="display: flex; justify-content: space-between;">
								<span><a href="detail?videoId=${item.id}"
									class="list-title title ajax h-1x">${item.title}</a></span> <span>
									<c:choose>
										<c:when test="${favlist != null}">
											...
											<c:choose>
												<c:when test="${init == '1'}">
													<a href="unlike?videoId=${item.id}"><i
														data-feather="thumbs-down"></i></a>&nbsp;&nbsp;&nbsp;
													<a href="sharevideo?videoId=${item.id}"><i
														data-feather="share-2"></i></a>
												</c:when>
												<c:otherwise>
													<a href="like?videoId=${item.id}"><i
		...
		<nav aria-label="Page navigation">
			<ul class="pagination justify-content-center">
				<li class="page-item ${indexPage>1?'':'disabled'}"><a
					class="page-link" href="home?page=${indexPage-1}"
					aria-label="Previous"> <span>&laquo;</span> <span
						class="sr-only">Previous</span>
				</a></li>
				<c:forEach begin="1" end="${numberPage}" var="i">
					<li class="page-item ${indexPage==i?'active':''}"><a
						class="page-link" href="home?page=${i}">${i}</a></li>
				</c:forEach>
				<li class="page-item ${indexPage == numberPage?'disabled':''}"><a
					class="page-link" href="home?index=${indexPage+1}"
					aria-label="Next"> <span>&raquo;</span> <span class="sr-only">Next</span>
				</a></li>
			</ul>
		</nav>
	</div>
